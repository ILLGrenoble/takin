...
mkdir -p ${INSTDIR}/usr/local/lib/python3.14/dist-packages/takin
...
# control file
echo -e "Package: takin\nVersion: 2.10" > ${INSTDIR}/DEBIAN/control
...
echo -e "Depends:" \
	...
	"libboost-iostreams1.90.0 (>=1.90.0)," \
	"libboost-regex1.90.0 (>=1.90.0)," \
	"libboost-program-options1.90.0 (>=1.90.0)," \
	"libboost-python1.90.0 (>=1.90.0)," \
...
# copy py modules
cp -v pymods/*                  ${INSTDIR}/usr/local/lib/python3.14/dist-packages/takin
...
strip -v ${INSTDIR}/usr/local/lib/python3.14/dist-packages/takin/*.so
